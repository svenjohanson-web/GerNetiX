...
test("external MQTT listener requires mutual TLS and device ACLs", () => {
  assert.match(config, /listener 8883 0\.0\.0\.0/);
  assert.match(config, /allow_anonymous false/);
  assert.doesNotMatch(config, /password_file/);
  assert.match(config, /cafile \/mosquitto\/config\/device-ca\.pem/);
  assert.match(config, /require_certificate true/);
  assert.match(config, /use_identity_as_username true/);
...
test("broker starts internal listeners while the MQTT certificate is pending", () => {
  assert.match(entrypoint, /MQTT TLS certificate or Device CA is not available/);
  assert.match(entrypoint, /awk '\/\^listener 8883/);
  assert.match(entrypoint, /cp "\$device_ca" \/tmp\/device-ca\.pem/);
  assert.match(entrypoint, /rm -f \/mosquitto\/data\/passwords/);
