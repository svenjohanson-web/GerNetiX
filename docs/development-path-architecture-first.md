...
Nach der Auswahl eines GerNetiX-Katalogboards oder einer eigenen Account-Boardversion zeigt die IDE dessen `default_instance_configuration.board_features` in derselben fachlichen Struktur wie das Provisioning: verwendete Komponente, Hardwareart, Treiber, Anschluss, Pin-Zuordnung und Groessenwert. Abweichungen von der gewaehlten Ausgangsversion werden hervorgehoben. Eine veraenderte Konfiguration bleibt bis zur ausdruecklichen Vergabe eines eigenen Boardnamens ein nicht speicherbarer Entwurf; danach legt Device Management eine unveraenderliche, accountgebundene Boardversion in PostgreSQL an. Der Project Server persistiert zusaetzlich einen vollstaendig aufgeloesten Snapshot in `build_config.board_configuration`, damit eine spaetere Account-Boardversion bestehende Projekte nicht rueckwirkend veraendert. Das globale Hardware-Catalog-Profil bleibt unveraendert und sein `base_board_profile_id` bildet weiterhin den physischen Build- und Inventar-Kompatibilitaetstyp.

Jede IoT-Device-Komponente besitzt in der IDE einen dauerhaften, vom aktuellen Realisierungsstand unabhaengigen Einstieg `Konfiguration/Uebersicht`. Die Uebersicht und der Projektbaum gliedern dieselben Themen an genau einer Stelle: Unter `Konfiguration/Hardware` liegen `Boardkonfiguration` und `Angeschlossene Komponenten`; unter `Konfiguration/Software` liegen `Funktionen`, `Treiberverwaltung`, `Webserver-Konfiguration` und `Webserver-Vorschau`. Die Uebersicht verlinkt auf diese vorhandenen Fachansichten und bildet keine zweite Konfigurationsquelle. Die `BoardConfigurationPlugin`-Komponente rendert in Provisioning und IDE dieselbe Boardauswahl, Ausstattungstabelle und Pinbearbeitung. Fehlt im Projekt noch ein Board, erscheint deshalb direkt diese Auswahl statt eines Verweises auf einen anderen Konfigurationsschritt. Ein unveraenderter GerNetiX- oder Account-Boardstand wird als Projektsnapshot gespeichert; geaenderte Katalogwerte muessen weiterhin benannt und als unveraenderliche Account-Boardversion gespeichert werden.
