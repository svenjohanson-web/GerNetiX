...
Der IDE-Projektbaum zeigt Quellcode immer im `Source`-Ordner der fachlich verantwortlichen Komponente. Komponentenpfade wie `Komponenten/IoT-Device 1/src/user_main.cpp` und noch vorhandene technische Projektpfade wie `src/main.cpp` werden dafuer nur in der Lesesicht unter `Komponenten/IoT-Device 1/Source` zusammengefuehrt; die PostgreSQL-Pfade werden nicht still verschoben oder dupliziert. Fuer neue Boardzuordnungen bleibt der komponentengebundene Pfad der Projekteinstieg. `user_target_path` beschreibt davon getrennt, an welche technische Stelle die Datei erst beim Erzeugen des BuildPackage projiziert wird.
