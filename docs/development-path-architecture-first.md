...
Nach der Auswahl eines Katalogboards zeigt die IDE dessen `default_instance_configuration.board_features` in derselben fachlichen Struktur wie das Provisioning: verwendete Komponente, Hardwareart, Treiber, Anschluss, Pin-Zuordnung und Groessenwert. Abweichungen vom Katalogstandard werden hervorgehoben. Eine veraenderte Konfiguration bleibt bis zur ausdruecklichen Vergabe eines eigenen Boardnamens ein nicht speicherbarer Entwurf; danach wird sie als projektgebundene `board_configuration` im `hardware-configuration`-View des Project Servers persistiert. Das globale Hardware-Catalog-Profil und die darauf verweisende `board_profile_id` bleiben unveraendert und bilden weiterhin den Build-Zieltyp.
