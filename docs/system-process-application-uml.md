...
- Der Code-Explorer folgt einem kontrollierten Coding-Agent-Ansatz mit OpenAI Responses Function Calling: Die IDE uebergibt nur Nutzeraufgabe und aktuellen Pfad. Das Modell nutzt danach serverseitig `search_project_sources` und `read_project_source`; Suchtreffer enthalten zunaechst keinen Dateiinhalt. Nur explizit gelesene Projektpfade duerfen als Aenderung vorgeschlagen werden. Eine feste Uebergabe der ersten 40, einer willkuerlichen Treffermenge oder aller Projektdateien ist nicht zulaessig; Schreibzugriffe bleiben bestaetigungspflichtig.
