...
- Die Flashbox ist ein kaufbares, inventarisierbares GerNetiX-Werkzeuggeraet und kein frei erfassbares Zielboard. Hardware Catalog beschreibt die Klasse `flashbox`, Webshop erzeugt den Kauf-/Claim-Kontext, Provisioning Tool bereitet Seriennummer und Device-Identitaet vor, Identity ordnet die konkrete Einheit dem Account-Inventar zu, Device Management fuehrt Ownership, Trust-State, Firmwarestatus und Revocation, Build-&-Deploy liefert getrennte signierte Manifeste fuer Flashbox-Selbstupdate, Zielgeraete-Flash und Recovery. Die Detailregeln stehen in [GerNetiX Flashbox - Systemzusammenspiel](flashbox-system-integration.md).
