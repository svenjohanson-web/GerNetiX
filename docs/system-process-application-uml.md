...
flowchart LR
  ...
  subgraph platformInfrastructure["Technische Infrastruktur"]
    ...
    linuxBuildWorker["Linux Build-Worker<br/>build_only + lokaler Cache<br/>WireGuard :4400"]
  end
...
| Linux Build-Worker | konfigurierbar, Standard 4400 | nur private WireGuard-Adresse | Eigenstaendiger Docker-Worker mit PlatformIO und lokalem Cache; Rolle `build_only`, kein MQTT und kein OTA-Signierschluessel |
...
| Build Worker Pool Router | Linux Build-Worker | Private HTTP-Verteilung ausschliesslich ueber WireGuard; Upstreams werden ueber VPS-Konfiguration zugelassen |
| Linux Build-Worker | Zentrales PostgreSQL | BuildJob-Heartbeat, Ziel-Lock, zentrale Statussicht und Artefakt-BLOBs ueber den WireGuard-gebundenen PostgreSQL-Port |
...
- Das Kamera-zu-Touchdisplay-Template besitzt ab Modellversion 16 einen ausfuehrbaren lokalen Durchstich mit zwei getrennten Firmware-Zielen: Die OV3660 liefert QVGA-JPEG-Frames, der Kamera-Host stellt den konfigurierten MJPEG-HTTP-Endpunkt sowie einen Einzelbild-Endpunkt bereit, und der Display-Client ruft die JPEG-Bilder per HTTP ab, dekodiert sie im PSRAM und zeichnet sie ueber den aus dem Board-Snapshot abgeleiteten ILI9341-SPI-Bus. Im initialen Geräte-AP-Modus bilden SSID, Clientbeitritt, IP, DHCP-Bereich, Firmware-Rollen und Endpunkt einen gemeinsamen Kommunikationssnapshot; im Infrastruktur-WLAN wird der Host ueber den daraus abgeleiteten lokalen mDNS-Namen gefunden. Projekt-MQTT ist fuer diesen Bildpfad deaktiviert. Der geschuetzte GerNetiX-OTA-Steuerkanal bleibt davon getrennt und kann provisionierten Inventar-Geraeten weiterhin signierte OTA-Auftraege melden. Projektbezogene ESP-IDF-Abhaengigkeiten werden nur in das jeweilige BuildPackage uebernommen und veraendern die stabile Basissoftware nicht global.
