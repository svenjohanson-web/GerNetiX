...
| Lokaler Forgejo-Buildadapter | kein Port | Entwicklerrechner | Duenn hinter `build.bat`; materialisiert dieselben softwareeinheitenbezogenen BuildPackages und verwendet `BuildPackageStore` sowie `FirmwareBuildJobRunner` des Workers. Produktquelle und geschuetzte Basissoftware werden nur im ignorierten BuildWorkspace zusammengefuehrt, nicht als zweite Git-Quellkopie. |
