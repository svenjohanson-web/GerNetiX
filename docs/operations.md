...
| Nutzeraktions-Zustell-Outbox | Umgesetzt | Identity speichert minimierte Events vor dem Versand in PostgreSQL, entfernt sie nach Erfolg und sendet bei Wiedererreichbarkeit oder Neustart begrenzt nach. Bei gleichzeitigem PostgreSQL-Ausfall ueberbrueckt eine begrenzte prozesslokale Notfallwarteschlange bis zur automatischen Nachlieferung. | Staging-Ausfall-/Recovery-Nachweis einschliesslich Prozessabsturzgrenze | Reporter- und PostgreSQL-State-Store-Tests |
| Nutzeraktions-Prozessmonitor | Umgesetzt | Read-only Auffaelligkeiten fuer `failed`, `timed_out` und `unhandled` aus Operations-PostgreSQL ueber den fest begrenzten Diagnosebefehl `user-action-alerts`; im kontrollierten Remote-Dev-Modus zusaetzlich sofortige minimierte Identity-Notfallspur ueber einen festen Loopback-Endpunkt, dedupliziert und mit vollstaendiger Vorgangs-ID | Diagnosewrapper auf Staging installieren, neues Desktop-Paket und kontrollierter Ausfall-/Recovery-Lauf | Prozessmonitor-, Identity-Routen- und Datenschutz-Contract-Test |
...
| Forgejo-Repository-Betrieb | Umgesetzt | Token-geschuetzte Operations-Sicht fuer System- und Projekt-Repositories mit Erreichbarkeit, Latenz, Git-/LFS-Groesse, Objektzahl, Builds, Artefakten, fehlenden Bindungen und lesender Orphan-Erkennung; getrennte Git-, LFS- und Artifact-Store-Quoten | Periodischer Staging-Lauf, letzter Backup-/Restore-Zeitpunkt und externer RPO/RTO-Nachweis | Project-Server-, Forgejo-Client-, Quoten-, Ausfall-, Konflikt- und Orphan-Contract-Tests |
| Forgejo Backup, Restore und Upgrade | Teilweise | Gemeinsamer Sicherungspunkt fuer Datenbank und `forgejo_data`, real bestandener lokaler Leerstand-Restore, sicherer Age-X25519-Vertrag und isolierter gepinnter Upgrade-/Doctor-Helfer | Externe getrennte Aufbewahrung sowie produktiver RPO/RTO- und freigegebener Versionsupgrade-Nachweis | Backup-/Restore-, Verschluesselungs- und Upgrade-Contract-Tests |
