...
| 2026-08-15 | Gefuehrte Acht-Satz-Einrichtung | Der echte tastenbasierte Satzablauf meldet Satz, Referenz, KEY2-Handlung, Fehler und Abschluss mit monotoner Revision ueber zwei begrenzte UART-Aktionen. Die oeffentliche Seite spiegelt den Boardzustand im 750-ms-Takt und spricht neue Schritte nach bewusster Aktivierung lokal vor; waehrend der Aufnahme bleibt sie stumm. Contract-, UI-Tests und vollstaendiger ESP32-S3-Build sind erfolgreich. | Nutzer flasht und prueft den vollstaendigen Acht-Satz-Ablauf auf dem Board. |
| 2026-08-18 | Gefuehrte Acht-Satz-Einrichtung – WebSerial | Die oeffentliche Seite spricht unter Chrome/Edge auf Windows und Linux direkt per WebSerial mit dem Board (kein lokaler Helper noetig); auf macOS ohne WebSerial bleibt der GerNetiX Serial Service der Fallback. Uebertragungswahl und Fehlercodes sind ueber Regressionstests abgesichert. | Nutzer prueft den vollstaendigen Acht-Satz-Ablauf ueber WebSerial an einem echten Windows/Chrome-Rechner mit angeschlossenem Board, inklusive eines bewussten Boardresets waehrend der Einrichtung. |
