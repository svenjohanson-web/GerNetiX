...
Der Desktop-Prozessmonitor besitzt neben der Prozesssicht eine persistierte `Schnittstellen-Statistik`. Instrumentierte Dienste schreiben Quelle, Ziel, normalisierte Route, Status und Dauer ueber den geschuetzten Ingest in `operations_interface_calls` der zentralen Operations-PostgreSQL. Die Ansicht liest ausschliesslich ein Aggregat der letzten 24 Stunden mit Aufrufen, Fehlern, mittlerer/maximaler Dauer und letztem Aufruf je Quelle-Ziel-Verbindung. Weder der Electron-Prozess noch sein Renderer greifen direkt auf PostgreSQL oder eine lokale Runtime-SQLite zu. Monitor-Healthchecks werden nicht mitgezaehlt.
...
- macOS-Build: `pnpm run dist:mac` (baut das Bundle und vergleicht danach alle ausgelieferten Quellen bytegenau mit dem aktuellen Stand)
...
Die VPS-Prozesskarten stammen aus einer getrennten, kurzlebigen SSH-Diagnoseabfrage. Sie umfasst mit `docker compose ps --all` auch gestoppte Container; der Monitor ergaenzt ausserdem jeden im aktuellen VPS-Katalog erwarteten, aber nicht gelieferten Prozess als `Nicht gestartet`. Fuer die zentrale Kontodatenbank reicht ein gruener Docker-Healthstatus nicht aus: Eine zweite feste read-only Diagnose fuehrt eine echte Abfrage gegen `gernetix_runtime.identity_user_accounts` aus. Scheitert sie, zeigt der Monitor ausdruecklich `Zentrale Kontodatenbank nicht erreichbar – Login nicht verfuegbar`. Die PostgreSQL-Karte besitzt eine feste Startaktion, die ausschließlich `runtime-postgres` idempotent per Compose startet und anschließend bis zur erfolgreichen Kontoabfrage wartet; freie Dienstnamen, Stoppen oder Entfernen sind nicht moeglich. Der lokale Identity-Healthcheck prueft die Datenbankverbindung ebenfalls; bei einer Stoerung zeigt auch die lokale Prozesskarte `PostgreSQL nicht erreichbar – Login nicht verfuegbar`. Vor dem Passkey-Dialog prueft die Login-Seite denselben Healthstatus, unterscheidet den Infrastrukturfehler von einem wirklich ungueltigen Passkey und behaelt die Vorgangs-ID bei. Schlaegt die VPS-Abfrage insgesamt fehl, zeigt der Monitor zuletzt bekannte oder erwartete Dienste ausschliesslich mit `Status unbekannt`, nennt den Fehler sowie Pruefzeitpunkt und gegebenenfalls den letzten erfolgreichen Nachweis. Ein alter gruener Containerstatus darf nicht als aktueller Zustand stehen bleiben. Die dauerhafte Portweiterleitung wird separat angezeigt und entscheidet ueber die Erreichbarkeit lokaler Tunnel-URLs wie `127.0.0.1:14600`.

Die App selbst oeffnet keinen HTTP-Port. In der lokalen Prozesssicht erscheint der kontrollierte Prozess eindeutig als `Identity Dev-Server` mit Status, PID und Port `4300`; ist er gestoppt, kann er dort direkt ueber `Dev-Server starten` gestartet werden. Die lokalen Start-/Stop-Aktionen steuern ausschliesslich diesen Identity-Listener und getrennt davon den build-only Docker-Worker auf seiner privaten WireGuard-Adresse. Die einzige VPS-Startaktion ist auf die zentrale Kontodatenbank `runtime-postgres` fest verdrahtet. PostgreSQL und die Domaenendienste bleiben auf dem VPS. Das Schliessen des letzten Monitorfensters beendet auch den Desktop-Monitor; separat gestartete lokale Prozesse und die VPS-Prozesse bleiben davon unberuehrt.
