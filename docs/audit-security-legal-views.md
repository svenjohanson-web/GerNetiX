...
| Projekte und Lernstand | Projekttitel, Beschreibung, Quellen, Hardware-Konfiguration, Build-Historie, Feedback | gemeinsame Service-SQLite, Volume `service_state`, Project Server | `user_id` und `project_id` binden Inhalt an den Tenant. Projektzugriff muss serverseitig gegen die Session geprueft werden. |
...
| Web-Push-Provider | accountgebundene Push-Subscription und technische Nachricht | Kein globaler Broadcast; Sicherheitsalarme nur an explizite Empfaengergruppe. |
