...
| Admin Access | `POST /api/admin/identity/support-recovery` | Zusaetzlich zur Admin-Session eine Passwort-Neuanmeldung im selben Aufruf; das Passwort wird vor dem Weiterreichen aus dem Rumpf entfernt und der Versuch auditiert. | `admin`; `admin_identity_recovery` und Neu-Authentifizierung. Ein uebernommenes Sitzungscookie allein reicht fuer die Kontowiederherstellung ausdruecklich nicht. |
...
| Community | `POST /api/community/notification-outbox/claim`, `.../{id}/complete`, `.../{id}/retry` | Signiertes Dienst-Token verlangt; eine mitgesendete Nutzerdelegation wird ausdruecklich mit `403` abgewiesen. | `internal-service`; `community.write`. Die Warteschlange bedient allein den Benachrichtigungs-Worker des Identity-Servers -- ein Konto hat hier nichts zu suchen. |
...
| `POST /api/platform/electronics-lab/assistant` | `user`; Identity-Session verpflichtend, eigenes Rate-Limit (`ELECTRONICS_LAB_AI_RATE_LIMIT`) und Audit-Ereignis je Anfrage. |
