...
function parseArgs(argv) {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    else if (["--host", "--local-port", "--remote-port", "--platform-port", "--remote-platform-port", "--hardware-catalog-port", "--remote-hardware-catalog-port"].includes(argument)) {
...
function sshTunnelArgs({ host, localPort, remotePort, platformPort, remotePlatformPort, hardwareCatalogPort, remoteHardwareCatalogPort }) {
  return [
    "-N",
    "-o", "BatchMode=yes",
    ...
    "-L", `${hardwareCatalogPort}:127.0.0.1:${remoteHardwareCatalogPort}`,
    host,
  ];
}
...
function main() {
  ...
  const hardwareCatalogPort = parsePort(args.hardwareCatalogPort || config.GERNETIX_STAGING_LOCAL_HARDWARE_CATALOG_PORT || 14910, "Lokaler Hardware-Catalog-Port");
  const remoteHardwareCatalogPort = parsePort(args.remoteHardwareCatalogPort || config.GERNETIX_STAGING_REMOTE_HARDWARE_CATALOG_PORT || 4910, "Remote-Hardware-Catalog-Port");
  const sshArgs = sshTunnelArgs({ host, localPort, remotePort, platformPort, remotePlatformPort, hardwareCatalogPort, remoteHardwareCatalogPort });
  const adminUrl = `http://127.0.0.1:${localPort}/admin/`;
  const platformUrl = `http://127.0.0.1:${platformPort}/app/dashboard/`;
  const hardwareCatalogUrl = `http://127.0.0.1:${hardwareCatalogPort}/api/hardware-catalog/`;

  process.stdout.write(`Staging-Plattform: ${platformUrl}\n`);
  process.stdout.write(`Staging-Admin: ${adminUrl}\n`);
  process.stdout.write(`Staging-Hardware-Katalog: ${hardwareCatalogUrl}\n`);
