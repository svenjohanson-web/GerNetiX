...
test("builds one aligned smoke plan without embedding credentials", () => {
  ...
  assert.equal(plan.api.environment.VUS, "10");
  assert.equal(plan.devices.arguments.includes("20"), true);
...
test("rejects non-loopback MQTT targets", () => {
  assert.equal(assertSafeMqttTarget("mqtt://127.0.0.1:1883"), "mqtt://127.0.0.1:1883");
