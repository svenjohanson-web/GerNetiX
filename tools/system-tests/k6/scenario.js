...
import { ROUTES, compactSummary, requestParamsForConfig, selectProject, settingUpdate } from "./lib/contracts.js";

const config = buildConfig(__ENV);
const flowFailures = new Rate("flow_failures");
const params = requestParamsForConfig(config);
...
function readSession(expectedUsername) {
  return group("identity session", () => {
    const response = http.get(url(ROUTES.session), params("session", "GET"));
    const payload = responseJson(response);
...
function readProjectList() {
  return group("project list", () => {
    const response = http.get(url(ROUTES.projectList), params("project_list", "GET"));
    const payload = responseJson(response);
...
function readProjectDetail(projectId) {
  return group("project detail", () => {
    const response = http.get(url(ROUTES.projectDetail(projectId)), params("project_detail", "GET", { project_id: projectId }));
    const payload = responseJson(response);
...
function saveProjectSetting(project) {
  return group("project settings CAS", () => {
    ...
    const readResponse = http.get(url(route), params("project_settings_read", "GET", { project_id: project.id }));
    ...
    const writeResponse = http.put(url(route), JSON.stringify(body), params("project_settings_save", "PUT", {
