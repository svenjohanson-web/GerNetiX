...
// A winding route through the complete board: start (top left) to target (bottom left).
constexpr uint8_t solution[36]={e|w,e|w,s|w,s|e,e|w,s|w,e|s,e|w,n|w,n|s,e|s,n|w,n|s,s|e,e|w,n|w,n|e,s|w,n|e,e|w,e|w,e|w,e|s,n|w,n|e,e|w,e|w,e|w,s|w,e|w,e|w,e|w,e|w,e|w,n|w};
...
bool PipePuzzle::solvedPath() const { bool seen[36]={};uint8_t q[36]={},head=0,tail=0;constexpr uint8_t start=0,target=30;if(!(tiles_[start]&w)||!(tiles_[target]&w))return false;q[tail++]=start;seen[start]=true;while(head<tail){uint8_t i=q[head++],row=i/size,col=i%size;const uint8_t dirs[4]={n,e,s,w};for(uint8_t d:dirs){if(!(tiles_[i]&d))continue;int rr=row,cc=col;if(d==n)--rr;else if(d==e)++cc;else if(d==s)++rr;else --cc;if(rr<0||cc<0||rr>=size||cc>=size)continue;uint8_t j=rr*size+cc;if(!seen[j]&&(tiles_[j]&opposite(d))){seen[j]=true;q[tail++]=j;}}}return seen[target]; }
