...
test("creates project with default source and lists it by user", async () => {
  ...
  assert.match((await service.getSource(project.project_id, "platformio.ini")).content, /Generated by GerNetiX/);
...
test("regenerates the visible platformio.ini whenever graphical build configuration is saved", async () => {
  ...
  assert.match((await service.getSource(project.project_id, "platformio.ini")).content, /platform = espressif8266/);
  ...
  const ini = (await service.getSource(project.project_id, "platformio.ini")).content;
