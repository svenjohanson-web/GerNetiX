...
test("creates project with default source and lists it by user", async () => {
  ...
  assert.equal(project.source_count, 5);
...
test("reports projected configuration files and creates no source diff for an unchanged dialog value", async () => {
  const service = createMemoryProjectServer();
  const project = await createDemoProject(service);
  const componentFeatures = {
    enabled: ["webserver", "measurement_chart"],
    webserver: {
      title: "Meine Messwerte",
      measurement_chart: true,
      measurement_label: "Temperatur",
      measurement_unit: "°C",
    },
  };

  const changed = await service.updateProject(project.project_id, {
    build_config: { ...project.build_config, component_features: componentFeatures },
  });
  assert.ok(changed.configuration_projection.changed_paths.includes("gernetix/configuration/software-features/firmware.json"));
  assert.match(
    (await service.getSource(project.project_id, "gernetix/configuration/software-features/firmware.json")).content,
    /Meine Messwerte/,
  );

  const unchanged = await service.updateProject(project.project_id, {
    build_config: { ...changed.build_config, component_features: componentFeatures },
  });
  assert.deepEqual(unchanged.configuration_projection.changed_paths, []);
  assert.ok(unchanged.configuration_projection.unchanged_paths.includes("gernetix/configuration/software-features/firmware.json"));
});
...
test("json repository persists projects, sources and build jobs across reload", async () => {
  ...
  assert.equal((await reloaded.listSources(project.project_id)).length, 5);
...
test("sqlite repository persists projects, template feedback, learning progress, sources and build jobs across reload", async () => {
  ...
  assert.equal(collectionCount(db, "project-server", "sources"), 5);
  ...
  assert.equal(tableCount(db, "project_server_sources"), 5);
...
test("deletes a project together with its stored project data", async () => {
  ...
  assert.equal(result.deleted.sources, 5);
