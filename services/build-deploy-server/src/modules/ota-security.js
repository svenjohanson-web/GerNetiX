...
class PostgresOtaAcknowledgementStore {
  ...
  async record(entry) { await this.upsert(entry); }
  async get(deployId) {
    return (await this.pool.query(`SELECT deploy_id,device_id,status,published_at::text AS published_at,
      acknowledged_at::text AS acknowledged_at,detail_json FROM build_deploy_ota_acknowledgements WHERE deploy_id=$1`,[deployId])).rows[0]||null;
  }
  async receive(topic,payload) {
    let detail;try{detail=JSON.parse(payload);}catch{return;}
    const deviceId=topic.split("/")[2]||detail.device_id||"";
    const commandId=detail.deploy_id||detail.deployId||detail.flashbox_job_id||detail.flashboxJobId||"";
    if(!deviceId||!commandId)return;
    await this.upsert({...detail,deploy_id:commandId,device_id:deviceId,status:detail.status||detail.state||"acknowledged",acknowledged_at:new Date().toISOString()});
  }
  async upsert(entry) {
    await this.pool.query(`INSERT INTO build_deploy_ota_acknowledgements
      (deploy_id,device_id,status,published_at,acknowledged_at,detail_json) VALUES ($1,$2,$3,$4,$5,$6)
      ON CONFLICT(deploy_id) DO UPDATE SET status=EXCLUDED.status,
      acknowledged_at=COALESCE(EXCLUDED.acknowledged_at,build_deploy_ota_acknowledgements.acknowledged_at),
      detail_json=EXCLUDED.detail_json`,
    [entry.deploy_id,entry.device_id,entry.status,entry.published_at||null,entry.acknowledged_at||null,entry]);
  }
}

module.exports = { PemOtaCommandSigner, SqliteOtaAcknowledgementStore, PostgresOtaAcknowledgementStore };
