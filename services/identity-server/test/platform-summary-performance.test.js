...
test("loads only requested platform summary sections and keeps selected dependencies concurrent", () => {
  ...
  assert.match(platformSummary, /const projectsPromise = needsProjects \? loadUserIdeProjects/);
  ...
  assert.match(platformSummary, /const projects = await projectsPromise;[\s\S]*const buildsPromise = sections\.has\("builds"\) \? loadProjectBuilds\(projects/);
...
test("loads projects in the critical bootstrap only for routes that need them", () => {
  ...
  assert.match(platformBootstrap, /const projects = sections\.has\("projects"\) \? await loadUserIdeProjects\(session\) : \[\]/);
  assert.match(platformBootstrap, /if \(sections\.has\("projects"\)\) payload\.projects = projects\.map\(toPlatformProject\)/);
  ...
  assert.match(app, /const initialRoute = routeName\(\);[\s\S]*Promise\.all\(\[refreshBootstrap\(initialRoute\), loadRouteAssets\(initialRoute\)\]\);[\s\S]*renderAll\(\);[\s\S]*renderRoute\(\{ contentRendered: true \}\);[\s\S]*hydratePlatformState\(initialRoute\)/);
...
test("loads development configuration only for development routes and preserves it across partial summaries", () => {
  ...
  assert.match(app, /hydrateRouteAfterNavigation\(activeRoute, \{ enterAfterHydration: waitingForAssets \}\)/);
