...
test("catalog includes the progressive microcontroller fundamentals course", () => {
  ...
  const pinMultiplexingLesson = course.lessons.find((lesson) => lesson.id === "45-final-pins-and-resources");
  assert.equal(pinMultiplexingLesson.title, "Abschlussprojekt: Pin-Multiplexing und Ressourcen planen");
  assert.equal(pinMultiplexingLesson.lab_url, "/technik-labs/?lab=pinmux&embedded=1");
  assert.match(pinMultiplexingLesson.summary, /Browser-Simulator[\s\S]*Arduino Nano oder ESP32[\s\S]*Oszilloskop und Logikanalysator/);
  assert.match(pinMultiplexingLesson.task, /ADC[\s\S]*PWM[\s\S]*UART[\s\S]*SPI-Signale/);
  assert.match(pinMultiplexingLesson.task, /feste oder flexible Routings/);
  assert.match(pinMultiplexingLesson.task, /absichtlichen Konflikt[\s\S]*Boot-[\s\S]*Flash-[\s\S]*PSRAM-[\s\S]*USB-[\s\S]*Debug-/);
  assert.match(pinMultiplexingLesson.code, /SPI SCLK\/MOSI\/MISO\/CS:[\s\S]*Alternative SPI-Belegung:[\s\S]*Bewusster Ressourcenkonflikt:[\s\S]*Konfliktlösung:/);
  assert.deepEqual(pinMultiplexingLesson.completion.must_not_contain, ["<eintragen>", "SENSOR_PIN = ;", "TASTER_PIN = ;", "LED_PIN = ;"]);
  const pinMultiplexingGuide = course.sources.find((source) => source.path === "PIN-MULTIPLEXING.md").content;
  assert.match(pinMultiplexingGuide, /Arduino-Nano-Projekt[\s\S]*ESP32-Projekt[\s\S]*Oszilloskop und Logikanalysator/);
  assert.match(pinMultiplexingGuide, /Boardmodell, Revision/);
  assert.match(pinMultiplexingGuide, /SCLK, MOSI, MISO und CS/);
  assert.match(pinMultiplexingGuide, /Grundlagen Gerätekommunikation/);
