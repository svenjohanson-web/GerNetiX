async function loadQuestion() {
  ...
  try {
    ...
    target.innerHTML = `<p class="eyebrow">${escapeQuestion((question.tags || []).join(" · ") || "Community")}</p><h1>${escapeQuestion(question.title)}</h1><p class="lead">${escapeQuestion(question.body)}</p><p class="community-public-meta">${Number(question.answer_count || 0)} Antworten · ${escapeQuestion(question.status || "offen")}</p>${renderPublicProjectSnapshot(question.project_snapshot)}<section class="community-public-answers"><h2>Antworten</h2>${visibleAnswers.length ? visibleAnswers.map((answer) => `<article><strong>${escapeQuestion(answer.author_label)}</strong><p>${escapeQuestion(answer.body)}</p></article>`).join("") : `<p class="lead">Noch keine veröffentlichte Antwort.</p>`}</section><a class="primary-action" href="/app/auth/?next=%2Fapp%2Fcommunity%2F">Eigene Frage stellen →</a>`;
...
function renderPublicProjectSnapshot(snapshot) {
  if (!snapshot) return "";
  const sources = snapshot.sources || [];
  return `<section class="community-public-answers"><h2>Projektstand zur Frage</h2><p class="community-public-meta">${escapeQuestion(snapshot.project_title)} · ${sources.length} Dateien · schreibgeschützt</p>${sources.map((source) => `<details><summary><code>${escapeQuestion(source.path)}</code></summary><pre><code>${escapeQuestion(source.content)}</code></pre></details>`).join("")}</section>`;
}
