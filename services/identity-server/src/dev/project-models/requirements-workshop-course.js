const modelData = require("./requirements-workshop-course.json");

function createRequirementsWorkshopCourseModel() {
  function createProject(project, step) {
    ...
    return project(
      ...
      definition.steps.map((item) => step(item.title, item.text, item.insight)),
      {
        ...
        development_lessons: modelData.development_lessons,
      },
    );
  }
...
module.exports = { createRequirementsWorkshopCourseModel };
