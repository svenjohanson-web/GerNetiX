...
function registerWebRoutes({
  ...
  registry.register({ method: "*", path: "/technik-labs/netzwerkanalysator/", handler: ({ res }) => redirect(res, "/technik-labs/?lab=vna") });
  registry.register({ method: "*", pattern: /^\/technik-labs\/(?:app\.js|styles\.css|lab-template-(?:contract|catalog)\.mjs|ai\/[^/]+\.mjs|labs\/[^/]+\.js|labs\/[^/]+\.mjs|environment-models\/[^/]+\.mjs|input-models\/[^/]+\.mjs|learning-circuits\/[^/]+\.mjs|learning-solver\/[^/]+\.mjs|peripherals\/[^/]+\.mjs|virtual-mcu\/[^/]+\.mjs)$/, handler: ({ res, url }) => serveStatic(res, virtualElectronicsLabDir, url.pathname.slice("/technik-labs".length), { versioned: url.searchParams.has("v") }) });
