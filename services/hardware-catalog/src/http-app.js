...
function createHttpApp(options) {
  ...
  return async function routeRequest(req, res) {
    ...
    if (req.method === "GET" && path === `${prefix}/capabilities`) {
      sendJson(res, 200, { items: await service.listCapabilities() });
      return;
    }

    const capability = path.match(new RegExp(`^${prefix}/capabilities/([^/]+)$`));
    if (req.method === "GET" && capability) {
      sendJson(res, 200, await service.getCapability(decodeURIComponent(capability[1])));
      return;
    }

    if (req.method === "GET" && path === `${prefix}/hardware-items`) {
      sendJson(res, 200, { items: await service.listHardwareItems(Object.fromEntries(url.searchParams.entries())) });
      return;
    }

    const hardwareItem = path.match(new RegExp(`^${prefix}/hardware-items/([^/]+)$`));
    if (req.method === "GET" && hardwareItem) {
      sendJson(res, 200, await service.getHardwareItem(decodeURIComponent(hardwareItem[1])));
      return;
    }

    if (req.method === "GET" && path === `${prefix}/processor-boards`) {
      sendJson(res, 200, { items: await service.listProcessorBoards() });
      return;
    }

    if (req.method === "GET" && path === `${prefix}/flashboxes`) {
      sendJson(res, 200, { items: await service.listFlashboxes() });
      return;
    }

    if (req.method === "GET" && path === `${prefix}/sensors`) {
      sendJson(res, 200, { items: await service.listSensors() });
      return;
    }

    if (req.method === "GET" && path === `${prefix}/board-feature-options`) {
      sendJson(res, 200, { items: await service.listBoardFeatureOptions() });
      return;
    }
