...
test("lists catalog capabilities and processor boards from catalog", async () => {
  ...
  assert.equal((await service.listProcessorBoards()).some((item) => item.hardware_item_id === "hardware.processor_board.esp32_2432s028"), true);
  ...
  const esp32_2432s028 = await service.getHardwareItem("hardware.processor_board.esp32_2432s028");
  assert.equal(esp32_2432s028.mcu_variant, "ESP32");
  assert.equal(esp32_2432s028.module_name, "ESP-WROOM-32");
  assert.equal(esp32_2432s028.capability_ids.includes("capability.display_output"), true);
  assert.equal(esp32_2432s028.capability_ids.includes("capability.touchscreen_input"), true);
  assert.equal(esp32_2432s028.default_instance_configuration.board_features.display.driver, "ili9341");
  assert.equal(esp32_2432s028.default_instance_configuration.board_features.display.pins.cs, 15);
  assert.equal(esp32_2432s028.default_instance_configuration.board_features.touch.driver, "xpt2046");
  assert.equal(esp32_2432s028.default_instance_configuration.board_features.touch.pins.cs, 33);
  assert.equal(esp32_2432s028.default_instance_configuration.hardware_test_status, "pending_controller_and_pinout_check");
  assert.equal(esp32_2432s028.platformio_build.environment, "esp32-2432s028-ili9341");
