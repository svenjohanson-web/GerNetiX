...
Der Service entscheidet vor einem KI-Aufruf, ob eine Datenquelle als Kontext genutzt werden darf. Auf dem VPS speichert er Grants, globale Policy, Prompt-Grundlagen, Architektur-Bausteine, kuratiertes Help-Wissen und Audit-Events in einer eigenen PostgreSQL-Datenbank mit pgvector. Die lokale SQLite-Datei bleibt als Entwicklungs-Fallback und einmalige Migrationsquelle erhalten.
...
- kuratierbares Help-Wissen mit pgvector-Suche; nur Treffer werden an den kostenkontrollierten OpenAI-Help-Agenten uebergeben
...
- `AI_CONTEXT_EMBEDDING_PROVIDER`: `openai` (Standard) oder optional `ollama`
- `AI_CONTEXT_EMBEDDING_BASE_URL`: Embedding-API, Standard `https://api.openai.com/v1`
- `AI_CONTEXT_EMBEDDING_MODEL`: Embedding-Modell, Standard `text-embedding-3-small`
- `AI_CONTEXT_EMBEDDING_DIMENSIONS`: Vektordimension, Standard `768`
- `AI_CONTEXT_OPENAI_API_KEY`: eigener serverseitiger OpenAI-Schluessel fuer Embeddings; ohne Schluessel bleibt der lexikalische Suchfallback aktiv
...
Die PostgreSQL-Schema- und Seed-Initialisierung wartet nicht auf den Embedding-Provider. Fehlende
...
- Help-Wissen hat den eigenen Source-Typ `help_knowledge`. Nur kuratierte Treffer duerfen nach AI-Usage-Preflight an OpenAI uebergeben werden; bei keinem Treffer antwortet Help ohne Modellaufruf, statt allgemeines oder externes Wissen zu verwenden.
