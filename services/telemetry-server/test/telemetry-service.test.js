...
test("derives account ownership server-side and only lists that account project data", async () => {
  ...
  assert.equal((await service.listMeasurements("acct-owner", "project-1", {})).length, 1);
  assert.equal((await service.listMeasurements("acct-other", "project-1", {})).length, 0);
});
...
test("persists threshold events before requesting project-scoped push", async () => {
  ...
  assert.equal((await service.listEvents("acct-owner", "project-1", {})).length, 1);
...
test("retention deletes expired telemetry while keeping current data", async () => {
  ...
  const result = await service.prune(new Date("2026-07-16T00:00:00Z"));
...
test("forwards transient runtime lines only after resolving device and project ownership", async () => {
  ...
  assert.equal((await service.listEvents("acct-owner", "project-1", {})).length, 0);
});
