...
const headerPath = path.join(projectRoot, "include/nexi/generated_story_audio.h");
const implementationPath = path.join(projectRoot, "src/generated_story_audio.cpp");
...
function symbolFor(id) {
  return `kStoryAudio${id.split(/[^a-zA-Z0-9]+/).filter(Boolean)
    .map((part) => part[0].toUpperCase() + part.slice(1)).join("")}`;
}

function renderValues(samples) {
  const lines = [];
  for (let offset = 0; offset < samples.length; offset += 24) {
    lines.push(`  ${Array.from(samples.subarray(offset, offset + 24)).join(", ")},`);
  }
  return lines.join("\n");
}
...
try {
  for (const pack of catalog.packs) {
    for (const story of pack.stories) {
      ...
      const samples = compactToPcm8(readMonoPcm16(wavPath, catalog.sampleRateHz));
      generated.push({
        id: story.id,
        symbol: symbolFor(story.id),
        samples,
...
constexpr uint16_t kSampleRateHz = ${catalog.sampleRateHz};

${generated.map(({ symbol }) =>
    `extern const int8_t ${symbol}[];\nextern const size_t ${symbol}SampleCount;`).join("\n\n")}

}  // namespace generated_story_audio
}  // namespace nexi
`;

const implementation = `// Generated by tools/generate-local-story-audio.js from
...
${generated.map(({ id, symbol, samples, sha256 }) => `// ${id}; sha256=${sha256}
alignas(4) const int8_t ${symbol}[] = {
${renderValues(samples)}
};
const size_t ${symbol}SampleCount = sizeof(${symbol});`).join("\n\n")}
...
process.stdout.write(`Generated ${generated.length} local stories (${generated
  .reduce((sum, item) => sum + item.samples.length, 0)} PCM8 samples).\n`);
